...
const EmpLogin = () => {
  const validationSchema = Yup.object().shape({
    empId: Yup.string().required('Company ID is required'),
  ...
  const onSubmit = (values, { setSubmitting, setErrors }) => {
    axios.post('http://16.171.229.175:3001/api/empLogin', values)
      .then(response => {
        ...
        localStorage.setItem('employeeId', decoded.empId);
      ...
      <div className="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
        ...
        <div className="mt-12 flex flex-col items-center">
          ...
          <div className="w-full flex-1 mt-8">
            ...
            <br></br>
            <Link to="/empForgotPassword" className="border-b border-gray-500 border-dotted text-center forgot-password"> Forgot Password </Link>
